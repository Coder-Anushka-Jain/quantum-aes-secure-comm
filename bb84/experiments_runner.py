...
from .key_rate_calculator import compute_key_rate_from_result, analyze_key_rate_statistics
...
def run_eve_sweep_with_key_rate(
    num_qubits: int,
    eve_probs: List[float],
    trials: int = 10,
    mu: float = 0.5,
    f: float = 1.16
) -> List[Dict]:
    """
    Run Eve probability sweep and compute secret key rates.
    
    Extension of run_eve_sweep that includes key rate analysis.
    ...
    Returns:
        List of dictionaries with QBER, key length, and key rate statistics
    """
    results = []
    ...
    for eve_prob in eve_probs:
        ...
        for _ in range(trials):
            ...
            # Compute key rate
            key_rate = compute_key_rate_from_result(result, mu=mu, f=f)
        ...
        results.append({
            ...
            'key_rate_stats': analyze_key_rate_statistics(key_rates)
        })
    
    return results


def run_adaptive_experiment_with_key_rate(
    num_rounds: int,
    qubits_per_round: int,
    eve_enabled: bool,
    eve_probability: float = 0.0,
    ...
    """
    Run adaptive experiment with key rate computation per round.
    
    Extension of run_adaptive_experiment that includes key rate tracking.
    
    Args:
        Same as run_adaptive_experiment, plus:
        mu: Mean photon number
        f: Error correction efficiency
    
    Returns:
        Dictionary with round results including key rates
    """
    # Initialize controller
    ...
    for round_num in range(num_rounds):
        ...
        # Store round result with key rate
        round_results.append({
            'round': round_num,
            'qber': qber,
            'key_length': key_length,
    ...
    return {
        ...
        'controller_stats': controller.get_statistics(),
        'final_probabilities': controller.probabilities.to_dict(),
        'key_rate_stats': analyze_key_rate_statistics([r['key_rate'] for r in round_results])
    }


def compare_static_adaptive_key_rates(
    num_rounds: int,
    qubits_per_round: int,
    eve_probability: float,
    adaptive_strategy: str = 'multi_metric',
    ...
    """
    Compare static vs adaptive with key rate analysis.
    
    Extension of run_static_vs_adaptive_comparison with key rates.
    
    Args:
        num_rounds: Number of rounds
        qubits_per_round: Qubits per round
        eve_probability: Eve's interception probability
        adaptive_strategy: Adaptive strategy to use
        trials: Number of trials
        mu: Mean photon number
        f: Error correction efficiency
    
    Returns:
        Comparison results including key rates
    """
    static_results = []
    adaptive_results = []
    ...
    for _ in range(trials):
        ...
        # Adaptive mode
        adaptive_trial = run_adaptive_experiment_with_key_rate(
            num_rounds=num_rounds,
            qubits_per_round=qubits_per_round,
            eve_enabled=True,
            eve_probability=eve_probability,
    ...
    for trial in static_results:
        for round_res in trial:
            static_qbers.append(round_res['qber'])
            static_key_lengths.append(round_res['key_length'])
            static_key_rates.append(round_res['key_rate'])
    ...
    for trial in adaptive_results:
        for round_res in trial['round_results']:
            adaptive_qbers.append(round_res['qber'])
            adaptive_key_lengths.append(round_res['key_length'])
            adaptive_key_rates.append(round_res['key_rate'])
    ...
    return {
        'static': {
            ...
            'mean_key_length': np.mean(static_key_lengths),
            'std_key_length': np.std(static_key_lengths),
            'mean_key_rate': np.mean(static_key_rates),
            'std_key_rate': np.std(static_key_rates),
            'key_rate_stats': analyze_key_rate_statistics(static_key_rates)
        },
        'adaptive': {
            'mean_qber': np.mean(adaptive_qbers),
            'std_qber': np.std(adaptive_qbers),
            'mean_key_length': np.mean(adaptive_key_lengths),
            'std_key_length': np.std(adaptive_key_lengths),
            'mean_key_rate': np.mean(adaptive_key_rates),
            'std_key_rate': np.std(adaptive_key_rates),
            'key_rate_stats': analyze_key_rate_statistics(adaptive_key_rates),
            'strategy': adaptive_strategy
        },
        'improvement': {
            'qber_reduction': (np.mean(static_qbers) - np.mean(adaptive_qbers)) / np.mean(static_qbers) * 100,
            'key_length_increase': (np.mean(adaptive_key_lengths) - np.mean(static_key_lengths)) / np.mean(static_key_lengths) * 100,
            'key_rate_increase': (np.mean(adaptive_key_rates) - np.mean(static_key_rates)) / max(np.mean(static_key_rates), 1e-6) * 100
        }
    }